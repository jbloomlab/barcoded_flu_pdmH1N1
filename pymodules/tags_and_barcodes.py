...
def parse_barcodes_supernatant(readiterator,
                               start,
                               end
                               ):
    """Get viral barcode sequence from reads sequenced from supernatant or
    second infection samples.
    ...
    sites = set(range(start, end))  # viral barcode sites
    nsites = len(sites)  # number of sites in viral barcode
    
    # parse barcode sequences for each read
    records = []
    for read in readiterator:
        read_id = read.name
